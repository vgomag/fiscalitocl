/**
 * WRITING-STYLE-ESM.JS — ESM re-export of writing-style constants & functions
 * ════════════════════════════════════════════════════════════════════════════
 * Identical content to writing-style.js but with ESM exports.
 * Used by ESM Netlify functions (export default) that cannot require() CJS.
 */
...
export const PRECISION_JURIDICA = `
...
export function detectEstamento(participants) {
...
export function getNormativeRegime(estamento) {
  switch (estamento) {
...
export const MODELO_SOBRESEIMIENTO = `
...
/* ═══ 6. FUNCIONES DE COMPOSICIÓN ═══ */
export function humanizePrompt(systemPrompt) {
  return systemPrompt + HUMAN_WRITING_STYLE;
}

export function buildSharedDirectives(participants, options) {
  const opts = options || {};
...
export function buildLightDirectives() {
