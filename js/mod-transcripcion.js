...
- Estamento denunciado/a: ${_fmtArr(caseObj.estamentos_denunciado)||'[ESTAMENTO]'}`;
  ...
  // Cargar participantes, resoluciones y fiscal desde la BD
  try{
    const[rParts,rRes]=await Promise.all([
      sb.from('case_participants').select('role,name,estamento,carrera,dependencia,rut').eq('case_id',cid).order('role'),
      sb.from('resoluciones').select('resolution_type,resolution_number,resolution_date,fiscal_designado,authority').eq('case_id',cid).order('created_at',{ascending:false}).limit(5)
    ]);

    if(rParts.data?.length){
      ctx+='\n\n## PARTICIPANTES REGISTRADOS';
      rParts.data.forEach(p=>{
        ctx+='\n- '+( p.role||'?')+': '+(p.name||'—');
        if(p.estamento)ctx+=' ('+p.estamento+')';
        if(p.rut)ctx+=' RUT: '+p.rut;
        if(p.carrera)ctx+=' — '+p.carrera;
        if(p.dependencia)ctx+=' — '+p.dependencia;
      });
      // Extraer fiscal para el encabezado
      const fiscal=rParts.data.find(p=>(p.role||'').toLowerCase().includes('fiscal'));
      if(fiscal)ctx+='\n- FISCAL INSTRUCTOR/A: '+fiscal.name;
      // Extraer actuario
      const actuario=rParts.data.find(p=>(p.role||'').toLowerCase().includes('actuari'));
      if(actuario)ctx+='\n- ACTUARIO/A: '+actuario.name;
    }

    if(rRes.data?.length){
      ctx+='\n\n## RESOLUCIONES';
      rRes.data.forEach(r=>{
        ctx+='\n- ['+(r.resolution_type||'Resolución')+'] '+(r.resolution_number||'—')+' ('+(r.resolution_date||'s/f')+')';
        if(r.fiscal_designado)ctx+=' Fiscal: '+r.fiscal_designado;
        if(r.authority)ctx+=' Autoridad: '+r.authority;
      });
      // Si no hay fiscal en participantes, tomarlo de resoluciones
      if(!rParts.data?.some(p=>(p.role||'').toLowerCase().includes('fiscal'))){
        const fiscalRes=rRes.data.find(r=>r.fiscal_designado);
        if(fiscalRes)ctx+='\n- FISCAL INSTRUCTOR/A (de resolución): '+fiscalRes.fiscal_designado;
      }
    }
  }catch(e){
    console.warn('Error cargando contexto del caso:',e);
  }
      ...
      if(raw.length<=MAX_CHUNK){
        ...
        for(let i=0;i<parts.length;i++){
          ...
          if(isLast){
            ...
            const body={model:'claude-sonnet-4-20250514',max_tokens:8000,system:systemPrompt,messages};
            const resp=await authFetch(ep,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
