...
/* ════════════════════════════════════════════════════════
   ...
   /.netlify/functions/structure (Claude Haiku)
   ════════════════════════════════════════════════════════ */
async function f11Paso2_Editar(){
  ...
  try {
    ...
    /* Fetch con timeout de 60 s para evitar cuelgues */
    const abortCtrl = new AbortController();
    const fetchTimeout = setTimeout(()=>abortCtrl.abort(), 60000);

    let resp;
    try {
      resp = await fetch('/.netlify/functions/structure', {
        method:'POST',
        ...
        body: JSON.stringify({
          ...
          mode: mode,
          caseContext: caseCtx,
          baseDocText: _f11DocText || ''
        }),
        signal: abortCtrl.signal
      });
    } catch(fetchErr){
      clearTimeout(fetchTimeout);
      if(fetchErr.name==='AbortError') throw new Error('Tiempo agotado (60 s). La función de edición no respondió. Verifica la conexión o intenta de nuevo.');
      throw new Error('Error de red: ' + (fetchErr.message||'no se pudo conectar con el servidor'));
    }
    clearTimeout(fetchTimeout);
    ...
    if(!resp.ok){
      /* Verificar si la respuesta es JSON o HTML (error de Netlify) */
      ...
      if(ct.includes('application/json')){
        ...
        console.error('[F11] Paso 2 respuesta no-JSON:', resp.status, bodySnippet.substring(0,500));
        if(resp.status===404) throw new Error('Función de edición no encontrada (404). Verifica que structure.js esté desplegado en Netlify.');
        if(resp.status===502||resp.status===503) throw new Error('Servidor temporalmente no disponible ('+resp.status+'). Intenta de nuevo en unos segundos.');
        if(resp.status===504) throw new Error('La función de edición tardó demasiado (timeout). Intenta con un texto más corto.');
    ...
    try { data = await resp.json(); } catch(parseErr){
      console.error('[F11] Paso 2 respuesta inválida:', parseErr);
      throw new Error('Respuesta inválida del servidor (no es JSON). Posible error de despliegue.');
    }
    if(!data.ok) throw new Error(data.error || 'Sin respuesta del modelo de IA');

    _f11EditedText = data.structuredText;
...
/* ════════════════════════════════════════════════════════
   ...
   /.netlify/functions/structure (modo 'con_expediente')
   ════════════════════════════════════════════════════════ */
async function f11Paso3_GenerarActa(){
  ...
  try {
    ...
    try {
      ...
      clearTimeout(fetchTimeout);
      if(fetchErr.name==='AbortError') throw new Error('Tiempo agotado (60 s). La función no respondió. Intenta de nuevo.');
      throw new Error('Error de red: ' + (fetchErr.message||'no se pudo conectar con el servidor'));
    }
    clearTimeout(fetchTimeout);
    ...
    if(!resp.ok){
      ...
      if(ct.includes('application/json')){
        ...
        const bodySnippet = await resp.text().catch(()=>'');
        console.error('[F11] Paso 3 respuesta no-JSON:', resp.status, bodySnippet.substring(0,500));
        if(resp.status===404) throw new Error('Función no encontrada (404). Verifica que structure.js esté desplegado.');
        if(resp.status===502||resp.status===503) throw new Error('Servidor no disponible ('+resp.status+'). Intenta de nuevo.');
        if(resp.status===504) throw new Error('Timeout del servidor. Intenta con texto más corto.');
        throw new Error('Error del servidor: HTTP '+resp.status);
      }
    }
    ...
    let data;
    try { data = await resp.json(); } catch(parseErr){
      console.error('[F11] Paso 3 respuesta inválida:', parseErr);
      throw new Error('Respuesta inválida del servidor (no es JSON). Posible error de despliegue.');
    }
    if(!data.ok) throw new Error(data.error || 'Sin respuesta del modelo de IA');

    _f11FinalActa = data.structuredText;
