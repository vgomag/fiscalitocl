...
async function processDiligenciaOCR(dilId){
  ...
  try{
    ...
    if(dil.drive_file_id){
      /* Step 1: Try text read first (works for Google Docs, text files) */
      try{
        const res=await fetch('/.netlify/functions/drive',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
      ...
      /* Step 2: If binary (PDF, images, Word), download as base64 and OCR with Claude */
      if(!extractedText){
        try{
          showToast('📥 Descargando archivo desde Drive…');
          ...
          if(dlRes.ok){
            ...
            if(dlData?.ok&&dlData.base64){
              showToast('🔍 Extrayendo texto con IA…');
              const mime=dlData.mimeType||'application/pdf';
              const isPdf=mime.includes('pdf');
              const isImage=mime.includes('image');

              /* Build Claude message with document/image content */
              let userContent=[];
              if(isPdf){
                userContent=[
                  {type:'document',source:{type:'base64',media_type:'application/pdf',data:dlData.base64}},
                  {type:'text',text:`Extrae TODO el texto de este documento PDF "${dil.file_name}" del expediente disciplinario. Mantén el formato original: párrafos, encabezados, numeración, tablas. Si algo es ilegible, indica [ilegible]. NO agregues comentarios propios, solo el texto extraído.`}
                ];
              } else if(isImage){
                userContent=[
                  {type:'image',source:{type:'base64',media_type:mime,data:dlData.base64}},
                  {type:'text',text:`Extrae TODO el texto visible en esta imagen "${dil.file_name}" del expediente disciplinario. Mantén el formato original. Si algo es ilegible, indica [ilegible]. Solo el texto extraído.`}
                ];
              } else {
                /* Word or other: send as document */
                userContent=[
                  {type:'document',source:{type:'base64',media_type:mime,data:dlData.base64}},
                  {type:'text',text:`Extrae TODO el texto de este documento "${dil.file_name}". Mantén el formato. Solo el texto extraído.`}
                ];
              }

              const ocrRes=await authFetch(CHAT_ENDPOINT,{
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
                  model:'claude-sonnet-4-20250514',
                  max_tokens:8000,
                  system:'Eres un experto en OCR y extracción de texto de documentos legales chilenos. Extrae todo el texto visible del documento proporcionado. Mantén el formato original (párrafos, listas, tablas). NO agregues comentarios, solo el texto extraído.',
                  messages:[{role:'user',content:userContent}]
                })
              });
              if(ocrRes.ok){
                const ocrData=await ocrRes.json();
                extractedText=(ocrData.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('')||'';
              }
            }
          }
        }catch(e){console.warn('Download+OCR failed:',e);}
      }
    }

    /* Fallback: if still no text */
    if(!extractedText){
      extractedText=`[No se pudo extraer texto de ${dil.file_name}. Verifique que el archivo exista en Drive y sea accesible.]`;
    }

    /* Generate summary */
    let aiSummary='';
    if(extractedText.length>100&&!extractedText.startsWith('[No se pudo')){
      try{
