...
async function processDiligenciaOCR(dilId){
  ...
  try{
    ...
    try{
      ...
      const _tout4=setTimeout(()=>_ctrl4.abort(),120000);
      try{
...
async function analyzeAllPending(){
  ...
  const needOcr=originals.filter(d=>!d.extracted_text || d.extracted_text.length < 200 || !d.is_processed);
  if(needOcr.length>0){
  ...
  if(needAnalysis.length>0){
    ...
    for(const d of needAnalysis){
      try{await analyzeExpediente(d.id);}catch(e){showToast(`❌ Error: ${e.message}`);}
      await new Promise(r=>setTimeout(r,1500));
    }
  } else if(pdfs.length>0 && childCount>0){
    showToast(`✅ ${pdfs.length} PDF(s) ya analizados (${childCount} diligencias)`);
  }
  if(needOcr.length===0 && needAnalysis.length===0) showToast('✅ Todas procesadas');
  else showToast('✅ Análisis completado');
}
