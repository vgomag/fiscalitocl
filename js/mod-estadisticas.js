...
async function loadStats(){
  ...
  try{
    ...
    cases.forEach(c=>{
      ...
      /* Género */
      if(isGenderCase(c.name,c.rol))generoCount.genero++;
...
function renderStats(d){
  ...
  return`<div style="padding:16px;display:flex;flex-direction:column;gap:16px;max-width:1200px;margin:0 auto">
...
function chartCard(id,title,desc){
  return`<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:14px">
    ...
    <div style="height:200px"><canvas id="${id}"></canvas></div>
  </div>`;
}
...
async function exportStatsCSV(){
  ...
  const headers=['Nombre','ROL','Estado','Tipo Procedimiento','Materia','Protocolo','Resultado','Fecha Denuncia','Fecha Recepción','Fecha Vista','Denunciante(s)','Denunciado/a(s)','Est. Denunciante','Est. Denunciado','Género'];
  const rows=allCases.map(c=>[
    c.name,c.rol||'',c.status||'',c.tipo_procedimiento||'',c.materia||'',c.protocolo||'',
    ...
    isGenderCase(c.name,c.rol)?'Género':'No Género',
  ]);
