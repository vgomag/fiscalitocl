...
function _propuestaLabel(c){
  /* Preferir el campo dedicado si existe */
  if(c.propuesta){
...
function _cumplimientoLabel(diasHabiles){
  ...
  /* Umbral típico legal: 6 meses ≈ 126 días hábiles. Mantenemos 126 como referencia. */
  return diasHabiles<=126?'Cumplido':'Cumplido fuera de plazo';
}
function _yearOf(...vals){
  for(const v of vals){
    if(!v)continue;
...
function _toExcelDate(v){
  /* Devuelve un objeto Date para que SheetJS lo escriba como fecha real */
  if(!v)return'';
...
/* Construye la hoja "Repositorio Procedimientos finalizados" */
function _xlsxBuildPlantillaTerminadosSheet(terminados){
  ...
  /* Ordenar terminados por fecha_resolucion_termino o fecha_vista (más antiguos primero) */
  ...
  /* Fila de totales (suma de "Días de tramitación") */
...
async function exportTerminadosTemplateXLSX(){
  ...
  try{
    ...
    /* Si hay campos faltantes, sugerir el enriquecimiento */
...
/* ═══════════════════════════════════════════════════════════
   ...
   Para cada caso terminado con drive_folder_url y campos faltantes
   (resolucion_termino / fecha_resolucion_termino / observaciones),
   lista la carpeta, identifica el documento más relevante
   (resolución término, vista, informe final), extrae el texto y
   pide a Claude un JSON estructurado para actualizar la BD.
   ═══════════════════════════════════════════════════════════ */
...
function _isCandidateFile(name){
  ...
  /* Documentos típicos donde aparecen los datos de cierre */
...
DOCUMENTOS (puede haber varios concatenados):
${(docsText||'').substring(0,20000)}

DEVUELVE SOLO JSON SIN MARKDOWN:
{
  "resolucion_termino": "número/identificador de la resolución que pone término al procedimiento (ej. '1091-2022-VRAC' o '1948-2024 Exenta') o null",
  ...
  "observaciones": "notas relevantes (máx 200 chars): designación de nuevo fiscal, reapertura, acumulación, número de tomos, etc., o null"
}`;
  const r=await authFetch(CHAT_ENDPOINT,{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({
      model:typeof CLAUDE_SONNET!=='undefined'?CLAUDE_SONNET:'claude-sonnet-4-20250514',
      max_tokens:800,
      system:'Eres un extractor de datos de documentos jurídicos chilenos. Responde SOLO con JSON válido sin markdown. Si un dato no está en los documentos, usa null.',
...
async function enrichTerminadosFromDrive(){
  ...
  panel.innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
      <strong>🔍 Completando desde Drive</strong>
      <button onclick="document.getElementById('${panelId}').remove()" style="background:none;border:none;font-size:16px;cursor:pointer;color:#666">×</button>
    </div>
    <div id="${panelId}-status" style="color:#374151;margin-bottom:6px">Iniciando…</div>
    <div style="background:#e5e7eb;border-radius:8px;height:8px;overflow:hidden;margin-bottom:6px">
      <div id="${panelId}-bar" style="background:#7c3aed;height:100%;width:0%;transition:width .3s"></div>
    </div>
    <div id="${panelId}-log" style="max-height:160px;overflow-y:auto;font-size:11px;color:#6b7280;font-family:var(--font-mono)"></div>`;
  ...
  const log=msg=>{const el=document.getElementById(panelId+'-log');if(el){el.innerHTML+=`<div>${esc(msg)}</div>`;el.scrollTop=el.scrollHeight;}};
  ...
  for(const c of t){
    ...
    try{
      ...
      /* 1) Listar carpeta y elegir candidatos (máx 3 archivos relevantes) */
      const files=await _driveListFolder(folderId);
      let candidates=files.filter(f=>_isCandidateFile(f.name));
      if(!candidates.length){
        /* Si nada coincide con la heurística, tomar los 2 archivos más recientes que sean docx/pdf */
      ...
      /* 2) Extraer texto de cada candidato (puede fallar individualmente) */
      const docs=[];
      ...
      /* 3) Pedir a la IA el JSON de cierre */
      const data=await _aiExtractCierreData(c,docs.join('\n\n---\n\n'));

      /* 4) Construir update solo con campos vacíos en BD */
      const update={updated_at:new Date().toISOString()};
      const setIfEmpty=(k,v)=>{if(v!==null&&v!==undefined&&v!==''&&!c[k])update[k]=v;};
      ...
      const{error:upErr}=await sb.from('cases').update(update).eq('id',c.id);
      if(upErr){log('⚠ '+label+': update error '+upErr.message);fail++;}
  ...
  /* Refrescar dashboard en segundo plano */
  setTimeout(()=>{try{loadStats();}catch{}},500);
}
