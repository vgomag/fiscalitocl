...
(function patchDriveQdrant() {
  ...
  try {
    /* BUG-FIX: la columna real es `drive_folder_id` (ver insert línea ~398),
       no `folder_id`. Antes el delete fallaba silenciosamente y la
       resincronización forzada no borraba nada. Agregamos también user_id
       como defensa en profundidad. */
    {
      ...
      if (user) {
        await sb.from('drive_processed_files')
          .delete()
          .eq('user_id', user.id)
          .eq('drive_folder_id', dq.selectedFolderId);
      }
    }
  ...
  try {
    ...
    /* Limpiar registros procesados — BUG-FIX: la columna real es
       `drive_folder_id` (no `folder_id`); además filtramos por user_id. */
    {
      const { data: { user } } = await sb.auth.getUser();
      if (user) {
        await sb.from('drive_processed_files')
