...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp  -- Bug-fix: search_path inmutable para prevenir injection
AS $$
DECLARE
  v_next INTEGER;
  v_prefix TEXT;
  ...
  v_formatted TEXT;
BEGIN
  -- Bug-fix SEC: validar que p_user_id coincida con usuario autenticado
  -- (previene escalada de privilegios donde un usuario genere contadores impersonando a otro)
  IF p_user_id IS DISTINCT FROM auth.uid() THEN
    RAISE EXCEPTION 'Unauthorized: p_user_id does not match authenticated user';
  END IF;

  -- Bug-fix SEC: validar que el usuario exista y el org_id no sea vacío
