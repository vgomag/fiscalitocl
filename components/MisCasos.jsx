              ...
              <tr key={caso.id} className="hover:bg-blue-50 border-b cursor-pointer transition">
                ...
                <td className="px-4 py-2 text-center">
                  {caso.drive_folder_url ? (
                    <a href={caso.drive_folder_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200 transition font-semibold text-sm">
                      ✓ Vinculado
                    </a>
                  ) : (
                ...
                <td className="px-4 py-2 text-center">
                  <button onClick={() => { setCasoSeleccionado(caso); setEditando(caso.id); setDriveLink(caso.drive_folder_url || ''); }} className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm">
                    Ver
                  </button>
                </td>
              </tr>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-96 overflow-y-auto">
            <div className="sticky top-0 bg-gray-100 px-6 py-4 flex justify-between items-center border-b">
              ...
              <button onClick={() => { setCasoSeleccionado(null); setEditando(null); setDriveLink(''); }} className="text-2xl text-gray-500 hover:text-gray-800">✕</button>
            </div>
                ...
                {editando === casoSeleccionado.id ? (
                  <div className="space-y-3">
                    <div>
                      ...
                      <input type="text" value={driveLink} onChange={(e) => setDriveLink(e.target.value)} placeholder="https://drive.google.com/drive/folders/..." className="w-full px-3 py-2 border border-gray-300 rounded mt-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => guardarDriveLink(casoSeleccionado.id)} className="flex-1 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 font-semibold">
                        ✓ Guardar enlace
                      </button>
                      <button onClick={() => { setEditando(null); setDriveLink(''); }} className="flex-1 px-4 py-2 bg-gray-400 text-white rounded hover:bg-gray-500">
                        Cancelar
                      </button>
                    </div>
                  </div>
                  ...
                  <div>
                    {casoSeleccionado.drive_folder_url ? (
                      <div className="space-y-3">
                        <p className="text-sm text-gray-700"><strong>Enlace guardado:</strong></p>
                        <a href={casoSeleccionado.drive_folder_url} target="_blank" rel="noopener noreferrer" className="block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-center font-semibold">
                          Abrir en Google Drive →
                        </a>
                        <button onClick={() => { setEditando(casoSeleccionado.id); setDriveLink(casoSeleccionado.drive_folder_url); }} className="w-full px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 text-sm">
                          Cambiar enlace
                        </button>
                      </div>
                    ) : (
                      <button onClick={() => { setEditando(casoSeleccionado.id); setDriveLink(''); }} className="w-full px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-semibold">
